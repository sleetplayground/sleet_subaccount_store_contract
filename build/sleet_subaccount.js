...
let SubAccountFactory = (_dec = NearBindgen({}), _dec2 = call({}), _dec3 = view(), _dec4 = call({
  payableFunction: true
}), _dec5 = call({}), _dec(_class = (_class2 = class SubAccountFactory {
  deposits = {};
  init() {
  ...
  create_subaccount({
    ...
    const contractId = currentAccountId();
    ...
    const promise = promiseBatchCreate(newAccountId);
    promiseBatchActionCreateAccount(promise);
    promiseBatchActionTransfer(promise, BigInt('1820000000000000000000')); // Convert string to BigInt
    promiseBatchActionAddKeyWithFullAccess(promise, publicKeyArray, 0); // Added nonce value

    // Deduct the deposit
    this.deposits[sender] = (currentDeposit - requiredDeposit).toString();
    log(`Created subaccount: ${newAccountId}`);
  }
}, _applyDecoratedDescriptor(_class2.prototype, "init", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "init"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "get_deposit", [_dec3], Object.getOwnPropertyDescriptor(_class2.prototype, "get_deposit"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "deposit", [_dec4], Object.getOwnPropertyDescriptor(_class2.prototype, "deposit"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "create_subaccount", [_dec5], Object.getOwnPropertyDescriptor(_class2.prototype, "create_subaccount"), _class2.prototype), _class2)) || _class);
function create_subaccount() {
  const _state = SubAccountFactory._getState();
  if (!_state && SubAccountFactory._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  const _contract = SubAccountFactory._create();
  if (_state) {
    SubAccountFactory._reconstruct(_contract, _state);
  }
  const _args = SubAccountFactory._getArgs();
  const _result = _contract.create_subaccount(_args);
  SubAccountFactory._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(SubAccountFactory._serialize(_result, true));
}
function deposit() {
  const _state = SubAccountFactory._getState();
  if (!_state && SubAccountFactory._requireInit()) {
...
function get_deposit() {
  ...
  const _args = SubAccountFactory._getArgs();
  const _result = _contract.get_deposit(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(SubAccountFactory._serialize(_result, true));
}
function init() {
  const _state = SubAccountFactory._getState();
...
export { SubAccountFactory, create_subaccount, deposit, get_deposit, init };
//# sourceMappingURL=sleet_subaccount.js.map
